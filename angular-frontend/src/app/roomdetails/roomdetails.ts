...
export class Roomdetails implements OnInit {
  ...
  calculateTotalPrice(): number {
    ...
    //convert it date
    const checkIn = new Date(this.checkInDate + 'T00:00');
    const checkOut = new Date(this.checkOutDate + 'T00:00');
  ...
  acceptBooking(): void {
    ...
    //Ensure the check in date and check out date are well formatted
    const formattedCheckInDate = this.checkInDate
      ? new Date(this.checkInDate).toLocaleDateString('en-CA')
      : '';
    const formattedCheckOutDate = this.checkOutDate
    ...
    this.api.bookRoom(booking).subscribe({
      next: (res: any) => {
        if (res.status === 200) {
          this.message =
            'Your Booking is Successful. An Email of your booking details and the payment link has been sent to you';
          setTimeout(() => {
            this.message = null;
